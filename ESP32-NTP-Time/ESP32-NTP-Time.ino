...
const char * ssid="IoT";
const char * wifipw="@IoT@S3nh@S3gur@";

void setTimezone(String timezone){
  Serial.printf("  Setting Timezone to %s\n",timezone.c_str());
  setenv("TZ", "AMT-4", 1);  //  Now adjust the TZ.  Clock settings are adjusted to show the new local time
  tzset();
}
...
void setup(){
  ...
  initTime("AMT+4AMST+3,M10.3.0,M2.3.0");   // Set for Melbourne/AU
  printLocalTime();
}
...
void loop() {
  ...
  setTimezone("AMT+4AMST+3,M10.3.0,M2.3.0");
  for(i=0; i<10; i++){
    delay(1000);
    printLocalTime();
  }
  ...
  setTime(2023,6,1,18,25,0,0);    // Set it to 1 minute before daylight savings comes in.
  ...
  setTime(2023,6,1,18,25,50,1);    // Set it to 1 minute before daylight savings comes in.  Note. isDst=1 to indicate that the time we set is in DST.
